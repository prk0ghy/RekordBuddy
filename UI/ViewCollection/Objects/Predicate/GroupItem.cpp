...
#include <ViewCollection/Predicate/GroupItem.hpp>
...
#include <ViewCollection/Predicate/DateItem.hpp>
...
struct NxA::RekordBuddy::GroupPredicateInspectorData
{
    ...
    std::unique_ptr<GroupItem> asGroup()
    {
        return std::make_unique<GroupItem>(childChoice, std::move(this->list), this->type);
    }
...
struct NxA::RekordBuddy::GroupPredicateInspector
{
                                   ...
                                   const String& value)
    {
        ...
        return std::make_unique<StringItem>(ChoiceData{ static_cast<ChoiceData::IdentityType>(typeID), {}, PredicateRule::NameMatch },
                                                  predicate);
    }
...
GroupItem::GroupItem(ChoiceData rule, std::list<std::unique_ptr<AbstractPredicateItem>>&& list, GroupMatchMethod method)
    : AbstractGroupItem{ std::move(rule), std::move(list), method }
{
}

GroupItem::GroupItem(ChoiceData rule, const Optional<TrackPredicate>& withPredicate)
    : AbstractGroupItem{ std::move(rule), GroupItem::listFromPredicate(withPredicate), GroupItem::methodFromPredicate(withPredicate) }
{
}

GroupMatchMethod GroupItem::methodFromPredicate(const Optional<TrackPredicate>& withPredicate)
{
...
std::list<std::unique_ptr<AbstractPredicateItem>>&& GroupItem::getValues()
{
    return std::move(this->value);
}

std::list<std::unique_ptr<AbstractPredicateItem>> GroupItem::listFromPredicate(Optional<TrackPredicate> withPredicate)
{
    if (!withPredicate.isValid()) {
        return {};
    }
...
QList<ChoiceData> GroupItem::getPossibleRules() const
{
...
std::unique_ptr<AbstractPredicateItem> GroupItem::createDefaultElement(ChoiceData rule) const
{
    if (rule.rule == PredicateRule::ChildRules) {
        return std::make_unique<GroupItem>(rule, nothing);
    }
        ...
        case Property::StringValueType: {
            switch (id) {
                ...
                case Property::TypeID::Tag: {
                    return std::make_unique<TagItem>(rule, TrackPredicate::comparisonWith(id, TrackPredicate::ComparisonOperator::Equal, ""));
                }
                case Property::TypeID::DateAdded:
                case Property::TypeID::DateReleased: {
                    return std::make_unique<DateItem>(rule, TrackPredicate::comparisonWith(id, TrackPredicate::ComparisonOperator::Equal,
                                                                                                 fromString(QDate::currentDate().toString())));
                }
                default:
                    break;
            }
            return std::make_unique<StringItem>(rule, TrackPredicate::comparisonWith(id, TrackPredicate::ComparisonOperator::Contains, ""));
        }
        case Property::IntegerValueType: {
            switch (id) {
                case Property::TypeID::MusicalKey:
                case Property::TypeID::FileType: {
                    return std::make_unique<EnumerationItem>(
                        rule, TrackPredicate::comparisonWith(id, TrackPredicate::ComparisonOperator::Equal, fromString(rule.options.first())));
                }
                default:
                    return std::make_unique<NumberItem>(rule,
...
Optional<TrackPredicate> GroupItem::asTrackPredicate() const
{
    ...
    for (auto&& element : this->value) {
        ...
        auto dp = dynamic_cast<Predicate*>(element.get());
