...
// -- Public Interface
class FilterPlaylist : public NXA_OBJECT
{
    #include <Base/ObjectDeclaration.ipp>

public:
    // -- Factory Methods
    static FilterPlaylist withNamePredicateWithParent(const String&, const Common::TrackPredicate&, Playlist&);

    // -- Instance Methods
    Optional<Common::TrackPredicate> maybePredicate() const;
    void setPredicate(const Optional<Common::TrackPredicate>&);
