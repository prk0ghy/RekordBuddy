...
// -- Public Interface
class SortingCriteria : protected NXA_OBJECT
{
    #include <Base/ObjectDeclaration.ipp>

public:
    // -- Constants
    enum class SortsABooleanTypeOfTag : boolean {
        No,
        Yes,
    };
    
    // -- Factory Methods
    static SortingCriteria withTypeInCollection(Common::Property::TypeID, Collection&);
    static SortingCriteria withTrackTagDescriptionInCollection(const String&, Collection&, SortsABooleanTypeOfTag);

    // -- Instance Methods
    Common::Property::TypeID tagTypeBeingSorted() const;
    boolean sortsABooleanTypeOfTag() const;
    String sortingPropertyDescription() const;

    void setAscending(boolean);

    Optional<Playlist> maybeParentPlaylist() const;
