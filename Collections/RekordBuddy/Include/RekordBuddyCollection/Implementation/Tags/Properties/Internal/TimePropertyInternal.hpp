...
inline constexpr bool operator==(const RekordBuddy::CollectionImplementation::PersistentTimeProperty& first,
                                 const RekordBuddy::CollectionImplementation::PersistentTimeProperty& second) noexcept
{
...
public:
    template<typename T>
    static std::shared_ptr<T> timePropertyWithValueInCollection(const Time& value, Collection& collection)
    {
        ...
        auto cacheMap = collection.p_timePropertyTagsCache().valueForKeyOrSetWith(RekordBuddy::CollectionImplementation::Property::propertyTypeIDForPersistentObjectType(RBSchema::TypeEnumeration<T>::value),
                                                                                  []() {
        ...
        newTag->setTimeValue(value);
        newTag->setSortingValue(value.stringValueInLocalTimeZoneUsingFormat(Time::defaultStringFormatWithJustDay));
...
// -- Private Functions
inline void p_deleteTimePropertyFromCollection(PersistentTimeProperty& property)
{
    ...
    auto sharedProperty = property.sharedFromThis<PersistentTimeProperty>();

    p_deleteTagFromCollection<PersistentTimeProperty>(property);

    for (auto&& parentTrack : parentTracksCopy) {
        if (parentTrack->objectID().isValid()) {
            parentTrack->removeTimePropertiesItem(sharedProperty);
        }
    }
}
