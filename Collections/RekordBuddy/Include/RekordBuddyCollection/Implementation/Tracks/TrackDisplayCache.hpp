...
// -- Public Interface
class TrackDisplayCache : protected NXA_OBJECT
{
    #include <Base/ObjectDeclaration.ipp>

public:
    // -- Factory Methods
    static TrackDisplayCache usingTrackInCollection(const Track&, Collection&);

    // -- Instance Methods
    void updateUsingTrack(const Track&);

    Array<String> stringValuesForTagOfType(Common::Property::TypeID) const;
    Array<integer32> integerValuesForTagOfType(Common::Property::TypeID) const;
    Array<DecimalNumber> decimalValuesForTagOfType(Common::Property::TypeID) const;
    Array<Time> timeValuesForTagOfType(Common::Property::TypeID) const;
