...
void PersistentTrackComments::bind(SourceBinder& parentBinder)
{
    ...
    parentBinder.bind(RBSchema::Type::TrackComments, RBSchema::Type::StringProperty);
}
void PersistentTrackComments::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackComments::deleteObject()
{
    if (isDeleted()) {
        return;
    }
