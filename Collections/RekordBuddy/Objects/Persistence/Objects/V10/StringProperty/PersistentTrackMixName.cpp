...
void PersistentTrackMixName::bind(SourceBinder& parentBinder)
{
    ...
    parentBinder.bind(RBSchema::Type::TrackMixName, RBSchema::Type::StringProperty);
}
void PersistentTrackMixName::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackMixName::deleteObject()
{
    if (isDeleted()) {
        return;
    }
