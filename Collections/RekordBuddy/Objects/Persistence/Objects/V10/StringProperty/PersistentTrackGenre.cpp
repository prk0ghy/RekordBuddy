...
template class std::shared_ptr<PersistentTrackGenre>;

template class NxA::Array<std::shared_ptr<PersistentTrackGenre>>;
void PersistentTrackGenre::bind(SourceBinder& parentBinder)
{
    PersistentStringProperty::bind(parentBinder);
    parentBinder.bind(RBSchema::Type::TrackGenre, RBSchema::Type::StringProperty);
}
void PersistentTrackGenre::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackGenre::deleteObject()
{
    if (isDeleted()) {
        return;
    }
