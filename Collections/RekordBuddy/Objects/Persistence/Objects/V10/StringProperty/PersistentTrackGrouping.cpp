...
template class std::shared_ptr<PersistentTrackGrouping>;

template class NxA::Array<std::shared_ptr<PersistentTrackGrouping>>;
void PersistentTrackGrouping::bind(SourceBinder& parentBinder)
{
    PersistentStringProperty::bind(parentBinder);
    parentBinder.bind(RBSchema::Type::TrackGrouping, RBSchema::Type::StringProperty);
}
void PersistentTrackGrouping::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackGrouping::deleteObject()
{
    if (isDeleted()) {
        return;
    }
