...
template class std::shared_ptr<PersistentTrackTitle>;

template class NxA::Array<std::shared_ptr<PersistentTrackTitle>>;
void PersistentTrackTitle::bind(SourceBinder& parentBinder)
{
    PersistentStringProperty::bind(parentBinder);
    parentBinder.bind(RBSchema::Type::TrackTitle, RBSchema::Type::StringProperty);
}
void PersistentTrackTitle::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackTitle::deleteObject()
{
    if (isDeleted()) {
        return;
    }
