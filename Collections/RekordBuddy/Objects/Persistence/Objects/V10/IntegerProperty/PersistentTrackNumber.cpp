...
template class std::shared_ptr<PersistentTrackNumber>;

template class NxA::Array<std::shared_ptr<PersistentTrackNumber>>;
void PersistentTrackNumber::bind(SourceBinder& parentBinder)
{
    PersistentIntegerProperty::bind(parentBinder);
    parentBinder.bind(RBSchema::Type::TrackNumber, RBSchema::Type::IntegerProperty);
}
void PersistentTrackNumber::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackNumber::deleteObject()
{
    if (isDeleted()) {
        return;
    }
