...
template class std::shared_ptr<PersistentTrackColor>;

template class NxA::Array<std::shared_ptr<PersistentTrackColor>>;
void PersistentTrackColor::bind(SourceBinder& parentBinder)
{
    PersistentIntegerProperty::bind(parentBinder);
    parentBinder.bind(RBSchema::Type::TrackColor, RBSchema::Type::IntegerProperty);
}
void PersistentTrackColor::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackColor::deleteObject()
{
    if (isDeleted()) {
        return;
    }
