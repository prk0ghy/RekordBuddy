...
void PersistentTrackSampleRate::bind(SourceBinder& parentBinder)
{
    ...
    parentBinder.bind(RBSchema::Type::TrackSampleRate, RBSchema::Type::IntegerProperty);
}
void PersistentTrackSampleRate::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackSampleRate::deleteObject()
{
    if (isDeleted()) {
        return;
    }
