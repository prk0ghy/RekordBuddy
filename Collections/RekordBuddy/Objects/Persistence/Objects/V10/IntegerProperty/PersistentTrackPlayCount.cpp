...
template class std::shared_ptr<PersistentTrackPlayCount>;

template class NxA::Array<std::shared_ptr<PersistentTrackPlayCount>>;
void PersistentTrackPlayCount::bind(SourceBinder& parentBinder)
{
    PersistentIntegerProperty::bind(parentBinder);
    parentBinder.bind(RBSchema::Type::TrackPlayCount, RBSchema::Type::IntegerProperty);
}
void PersistentTrackPlayCount::faultObject()
{
    if (isFaulted()) {
        return;
    }
...
void PersistentTrackPlayCount::deleteObject()
{
    if (isDeleted()) {
        return;
    }
