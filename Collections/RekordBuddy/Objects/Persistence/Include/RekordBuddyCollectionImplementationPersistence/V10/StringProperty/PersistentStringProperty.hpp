...
class PersistentStringProperty : public GenericPersistentObject<RBSchema>
{
...
protected:
    ...
    PersistentStringProperty(RBSchema::Type type, Context context)
        : PersistentStringProperty{ObjectID{type}, std::move(context)}
    {NXA_ASSERT_TRUE(RBSchema::typeIs(type, TypeEnumeration<PersistentStringProperty>::value));}
    
public:
    PersistentStringProperty(ObjectID id, Context context)
        : GenericPersistentObject<RBSchema>{id, context},
          stringValueAttribute{},
          manySortingPlaylists{std::make_unique<decltype(manySortingPlaylists)::element_type>(this, &PersistentStringProperty::loadSortingPlaylists)},
          manyParentTracks{std::make_unique<decltype(manyParentTracks)::element_type>(this, &PersistentStringProperty::loadParentTracks)}
    {NXA_ASSERT_TRUE(RBSchema::typeIs(id.objectType(), TypeEnumeration<PersistentStringProperty>::value));}
    ~PersistentStringProperty() override = default;
    PersistentStringProperty(const PersistentStringProperty&) = delete;
    PersistentStringProperty(PersistentStringProperty&&) = default;
    PersistentStringProperty& operator=(const PersistentStringProperty&) = delete;
    
    
    template<typename T = PersistentStringProperty>
        static Array<std::shared_ptr<T>> fetchByStringValue(Context context, String query, QueryOperation op = QueryOperation::Equals)
        {
            static_assert(RBSchema::typeIs(TypeEnumeration<T>::value, TypeEnumeration<PersistentStringProperty>::value),
                          "Must only query PersistentStringProperty or subtypes");
            
            // -- unindexed
            return context->template fetchObjectsByAttribute<PersistentStringProperty, T>("stringValue",
                                                                                          TypeEnumeration<T>::value,
                                                                                          &PersistentStringProperty::stringValueAttribute,
                                                                                          nothing,
                                                                                          query,
                                                                                          op);
        }
