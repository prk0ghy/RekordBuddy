...
inline constexpr bool operator==(const PersistentTrackFileType& first, const PersistentTrackFileType& second) noexcept
{
...
#define NXA_OBJECT_CLASS                                        TrackFileType
...
TrackFileType TrackFileType::withValueInCollection(AudioFileType value, Collection& collection)
{
    return TrackFileType{ IntegerPropertyInternal::integerPropertyWithValueInCollection<PersistentTrackFileType>(static_cast<integer32>(value), collection) };
}

// -- Instance Methods

AudioFileType TrackFileType::value() const
{
    return static_cast<AudioFileType>(nxa_const_internal->integerValue());
}
