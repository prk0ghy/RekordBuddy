...
inline constexpr bool operator==(const PersistentTrackBpm& first, const PersistentTrackBpm& second) noexcept
{
...
#define NXA_OBJECT_CLASS                                        TrackBpm
#define NXA_OBJECT_BASE_CLASS                                   DecimalProperty
...
TrackBpm TrackBpm::withValueInCollection(const DecimalNumber& value, Collection& collection)
{
    return TrackBpm{ DecimalPropertyInternal::decimalPropertyWithValueInCollection<PersistentTrackBpm>(value, collection) };
}

TrackBpm TrackBpm::withValueInCollection(const String& value, Collection& collection)
{
    return TrackBpm{ DecimalPropertyInternal::decimalPropertyWithValueInCollection<PersistentTrackBpm>(value, collection) };
}
...
DecimalNumber TrackBpm::valueInBeatsPerMinute() const
{
    return nxa_const_internal->decimalValue();
}
