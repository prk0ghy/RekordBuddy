...
inline constexpr bool operator==(const PersistentTrackFileSize& first, const PersistentTrackFileSize& second) noexcept
{
...
#define NXA_OBJECT_CLASS                                        TrackFileSize
...
TrackFileSize TrackFileSize::withValueInCollection(count value, Collection& collection)
{
    return TrackFileSize{ IntegerPropertyInternal::integerPropertyWithValueInCollection<PersistentTrackFileSize>(static_cast<integer32>(value), collection) };
}

// -- Instance Methods

count TrackFileSize::valueInBytes() const
{
